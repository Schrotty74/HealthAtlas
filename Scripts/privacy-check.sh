...
fail=0
forbidden='(/Users/|/Volumes/|/private/var/|/tmp/|health export|Health Export|access_token|client_secret|PRIVATE KEY|BEGIN RSA|BEGIN OPENSSH|\.sqlite$|\.db$|\.jsonl$)'
...
if find . -type f \( -name '*.dmg' -o -name '*.zip' -o -name '*.log' -o -name '*.ips' \) -not -path './.git/*' | rg -q .; then
