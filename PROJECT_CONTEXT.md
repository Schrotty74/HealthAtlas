...
- Die aktuelle öffentliche Final-Version ist `v1.0.1` mit ZIP, DMG und SHA-256-Dateien. Sie behebt die Darstellung rechteckiger Hintergrundanteile an abgerundeten Card-Ecken. Die aktuelle öffentliche Vorabversion ist `Beta 1.5.0` mit technischem Tag `v1.5.0-beta`. Sie enthält das zusätzliche Theme Schwarz & Weiß, dessen native neutrale Flächen der hellen oder dunklen macOS-Darstellung folgen, sowie verlässliche Theme-Wechsel, lesbare Kontraste und wieder aktivierte dezente Kartenanimationen für Clear Glass und Schwarz & Weiß. Der lokale Import bis 5 GiB, die Quellen-, Dubletten- und Überlappungslogik, die überarbeitete Oberfläche und die korrigierte Scrollleisten-Anzeige bleiben enthalten.
- Die Arbeitskopie behält den vorgemerkten Stand, aus dem Beta 1.5.0 erstellt wurde. `dev` bleibt ohne Remote-Branch. Den Stand nicht ohne ausdrücklichen Auftrag verwerfen oder in `dev` committen. Vor weiterer Arbeit zuerst `git status` und den Vergleich mit `beta` prüfen; eine weitere Beta entsteht erst aus neuen, ausdrücklich beauftragten Änderungen.
...
| `beta` | öffentliche Vorabversion auf GitHub | enthält die veröffentlichte Vorabversion `Beta 1.5.0` |
...
- Bestätigter offener Punkt: Das Beta-Skript bestimmt die vorherige Beta derzeit aus lokalen Tags. Fehlt ein aktueller Tag lokal, können die Release Notes auf eine ältere Beta verweisen. Vor der nächsten Beta muss die Ermittlung auf den tatsächlichen GitHub-Tag beziehungsweise das veröffentlichte Release umgestellt und geprüft werden.
