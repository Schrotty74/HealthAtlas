...
- Die Arbeitskopie behält den vorgemerkten Stand, aus dem Beta 1.3.0 erstellt wurde. `dev` selbst bleibt absichtlich ohne Remote-Branch. Den Stand nicht ohne ausdrücklichen Auftrag verwerfen oder in `dev` committen. Vor weiterer Arbeit zuerst `git status` und den Vergleich mit `beta` prüfen; eine weitere Beta entsteht erst aus neuen, ausdrücklich beauftragten Änderungen.
