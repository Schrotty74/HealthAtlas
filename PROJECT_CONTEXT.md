...
- Die aktuelle öffentliche Final-Version ist `v1.0.1` mit ZIP, DMG und SHA-256-Dateien. Sie behebt die Darstellung rechteckiger Hintergrundanteile an abgerundeten Card-Ecken. Die aktuelle öffentliche Vorabversion ist `Bugfix 1.5.4` mit technischem Tag `v1.5.4-beta`. Sie erweitert die Streaming-Importprüfung für gültige Apple-Health-Varianten, darunter UTF-8-BOM, XML-Kopf, interne `DOCTYPE`, Kommentare, CDATA, unbekannte Erweiterungselemente und Records mit untergeordnetem Inhalt. Fehlgeformte XML-Strukturen bleiben abgewiesen; die kopierbare, datensparsame Diagnose enthält bei Stream-Parserfehlern zusätzlich Zeile und Spalte. Auf GitHub sind für diesen Testbuild ausschließlich ZIP und DMG angehängt. Der vorherige Bugfix 1.5.1 importiert gültige Apple-Health-Exporte mit interner XML-`DOCTYPE` wieder direkt und aus ZIP-Archiven. Das zusätzliche Theme Schwarz & Weiß, dessen native neutrale Flächen der hellen oder dunklen macOS-Darstellung folgen, verlässliche Theme-Wechsel, lesbare Kontraste und dezente Kartenanimationen für Clear Glass und Schwarz & Weiß bleiben enthalten.
