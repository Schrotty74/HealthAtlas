...
- Die aktuelle öffentliche Final-Version ist `v1.0.1` mit ZIP, DMG und SHA-256-Dateien. Sie behebt die Darstellung rechteckiger Hintergrundanteile an abgerundeten Card-Ecken. Die aktuelle öffentliche Vorabversion ist der Diagnose-Testbuild `Bugfix 1.5.2` mit technischem Tag `v1.5.2-beta`. Er enthält für fehlgeschlagene Apple-Health-Importe eine kopierbare, datensparsame Diagnose, damit verbleibende Importfehler über Issue #13 eingegrenzt werden können. Auf GitHub sind für diesen Testbuild ausschließlich ZIP und DMG angehängt. Der vorherige Bugfix 1.5.1 importiert gültige Apple-Health-Exporte mit interner XML-`DOCTYPE` wieder direkt und aus ZIP-Archiven. Das zusätzliche Theme Schwarz & Weiß, dessen native neutrale Flächen der hellen oder dunklen macOS-Darstellung folgen, verlässliche Theme-Wechsel, lesbare Kontraste und dezente Kartenanimationen für Clear Glass und Schwarz & Weiß bleiben enthalten.
...
| `beta` | öffentliche Vorabversion auf GitHub | enthält die veröffentlichte Vorabversion `Bugfix 1.5.2` |
