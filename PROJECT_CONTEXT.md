...
- Die aktuelle öffentliche Final-Version ist `Final 2.0.0` mit technischem Tag `v2.0.0`, ZIP, DMG und SHA-256-Dateien. Sie enthält den bytebasierten Fortschrittsbalken beim direkten XML- und ZIP-Import ohne Restzeitschätzung. Bei ZIP-Dateien dient ausschließlich die unkomprimierte Größe von `Export.xml` als Prozentbasis. Die Streaming-Parserdiagnose verfolgt Zeile und Spalte inkrementell, damit Importe mit vielen Records keine Position für jeden Record erneut berechnen. Die bisherige Streaming-Importprüfung für gültige Apple-Health-Varianten bleibt enthalten, darunter UTF-8-BOM, XML-Kopf, interne `DOCTYPE`, Kommentare, CDATA, unbekannte Erweiterungselemente und Records mit untergeordnetem Inhalt. Fehlgeformte XML-Strukturen bleiben abgewiesen; die kopierbare, datensparsame Diagnose enthält bei Stream-Parserfehlern zusätzlich Zeile und Spalte. Die aktuelle öffentliche Vorabversion ist `Beta 1.6.0` mit technischem Tag `v1.6.0-beta`. Das zusätzliche Theme Schwarz & Weiß, dessen native neutrale Flächen der hellen oder dunklen macOS-Darstellung folgen, verlässliche Theme-Wechsel, lesbare Kontraste und dezente Kartenanimationen für Clear Glass und Schwarz & Weiß bleiben enthalten.
- `dev` bleibt ohne Remote-Branch. Vor weiterer Arbeit zuerst `git status` und den Vergleich mit `beta` prüfen; eine weitere Beta entsteht erst aus neuen, ausdrücklich beauftragten Änderungen.
...
| `beta` | öffentliche Vorabversion auf GitHub | enthält die veröffentlichte Vorabversion `Beta 1.6.0` |
| `main` | Final-Linie auf GitHub | enthält die ausdrücklich freigegebene Final-Version `Final 2.0.0` |
...
- Die startbare Dev-App heißt ausschließlich `HealthAtlas Dev.app` und liegt unter `dist/local-test/HealthAtlas-Development/HealthAtlas Dev.app`; auch ein direkter erfolgreicher Xcode-Dev-Build aktualisiert genau dieses Bundle. Öffentliche Beta-Pakete enthalten `HealthAtlas Beta.app`, Final-Pakete `HealthAtlas.app`; diese Channel-Namen nicht austauschen.
- Nach jeder Änderung im Abschluss immer den Link zur passenden App-Variante angeben: Dev für lokale Entwicklungsänderungen, Beta für Beta-Veröffentlichungen und Final für Final-Veröffentlichungen. Keine App, insbesondere nicht Dev, ohne ausdrücklichen Auftrag automatisch öffnen.
...
- Auf einem sauber geklonten Mac vergleicht `Scripts/bootstrap-local-dev.sh` die dokumentierten öffentlichen Versionen von `origin/beta` und `origin/main`. Es legt den ausschließlich lokalen Branch `dev` aus der neueren Linie an, ohne einen Remote-Tracking-Branch einzurichten, und wechselt anschließend zu `dev`. Bei gleichen oder nicht lesbaren Versionen bricht es sicher ab.
- Nach einer erfolgreich veröffentlichten Beta erstellt das Beta-Skript zusätzlich einen gleichwertigen lokalen Commit auf `dev`; dieser Commit wird nie gepusht. Ein Final kehrt nach der Veröffentlichung zu `dev` zurück und überschreibt keine späteren lokalen Dev-Commits.
- Vor dem Erzeugen des Beta-Commits aktualisiert dieses Skript die Beta-Zeile in `PROJECT_CONTEXT.md` automatisch auf die tatsächlich angeforderte Beta- oder Bugfix-Version; diese Korrektur wird Teil desselben Beta-Commits.
- `Scripts/publish-beta-as-final.sh` erstellt einen Final-Snapshot `X.0.0` standardmäßig aus der freigegebenen `beta`-Linie oder mit `--from-dev` direkt aus `dev` auf `main`. Es prüft den lokalen `dev`-Branch vor dem Release, aktualisiert die bekannten Final-Referenzen in `PROJECT_CONTEXT.md` und `NEXT_STEPS.md` im Final-Snapshot und lädt den neuen Final-Tag anschließend zur Commit-Prüfung nach. Die getrennten Branch-Historien bleiben dabei erhalten.
