...
- Die aktuelle öffentliche Final-Version ist `v1.0.1` mit ZIP, DMG und SHA-256-Dateien. Sie behebt die Darstellung rechteckiger Hintergrundanteile an abgerundeten Card-Ecken. Die nächste öffentliche Vorabversion ist `Beta 1.6.0` mit technischem Tag `v1.6.0-beta`. Sie ergänzt beim direkten XML- und ZIP-Import einen bytebasierten Fortschrittsbalken ohne Restzeitschätzung. Bei ZIP-Dateien dient ausschließlich die unkomprimierte Größe von `Export.xml` als Prozentbasis. Die Streaming-Parserdiagnose verfolgt Zeile und Spalte inkrementell, damit Importe mit vielen Records keine Position für jeden Record erneut berechnen. Die bisherige Streaming-Importprüfung für gültige Apple-Health-Varianten bleibt enthalten, darunter UTF-8-BOM, XML-Kopf, interne `DOCTYPE`, Kommentare, CDATA, unbekannte Erweiterungselemente und Records mit untergeordnetem Inhalt. Fehlgeformte XML-Strukturen bleiben abgewiesen; die kopierbare, datensparsame Diagnose enthält bei Stream-Parserfehlern zusätzlich Zeile und Spalte. Auf GitHub sind für diesen Testbuild ausschließlich ZIP und DMG angehängt. Das zusätzliche Theme Schwarz & Weiß, dessen native neutrale Flächen der hellen oder dunklen macOS-Darstellung folgen, verlässliche Theme-Wechsel, lesbare Kontraste und dezente Kartenanimationen für Clear Glass und Schwarz & Weiß bleiben enthalten.
